...
- **Consumers, later:** each screen's own agent starts writing its real state into the library as it's built — the Day Plan card is the first consumer (item 15, D55); the Context Engine (item 16 A) is next.
...
- **A — Awareness layer.** The load-bearing idea: no agent knows what the owner is currently doing, so every planner guesses. **`Context_Engine_Agent`** is the collector everything else reads — WakaTime (item 14, not wired yet), Google Calendar (D23), `git log --since` today, each screen's health endpoint — written into the library (item 2, D40). **Rule 8 applies hard:** an unreachable source is written as unreachable, never carried over from the last poll. Then `Time_Analyst_Agent` (evening gap report, hands to `Day_Planner_Agent`), `Pattern_Learner_Agent` (needs ~6 weeks of real history; reports "not enough data" until then), `Focus_Guard_Agent` (a comparison, not a judgment — no model call). The custom Pomodoro tracker stays unspec'd until real logged data exists.
